...
/**
 * @file dict.c
 * @brief Implementation of the dict_t using open addressing and incremental
 rehashing.
 ...
 * - A hybrid probing sequence (Quadratic for the first 32 attempts, then
 Linear) to reduce clustering.
 * - Incremental rehashing to distribute the resizing cost over multiple
 operations.
...
#define GET_STATE(tagged_ptr)                                                  \
  ((NodeState)((uintptr_t)(tagged_ptr) & STATE_MASK))
...
/**
 * @brief Searches for a value associated with a key in a specific internal
 array.
 * @internal
 *
 * Uses a hybrid probing strategy:
 * 1. Quadratic probing `(i*i)` for the first 32 attempts to spread out
 clusters.
 * 2. Linear probing `(+i)` thereafter to ensure all slots are eventually
 visited.
...
_dict_search(
  ...
  size_t index =
      hash_value & (array->size - 1); // This work because the size is pow of 2
  ...
  do {
    ...
    if (i < 32) {
      ...
      next_index = (start_idx + next_i) & (array->size - 1);
    }
...
_dict_change_state(
  ...
  size_t index =
      hash_value & (array->size - 1); // self->hash(key) % array->size;
 ...
 * @return DICT_ADDED on new insertion, DICT_UPDATED on update, DICT_ERR on
 failure.
...
_dict_insert(
  ...
  do {
    ...
    if (likely(state_tagged == USED &&
               self->compare(element.key, current_key) == 0)) {
...
dict_set(
  ...
  } else if (self->array[0].num_elements >=
             (size_t)((double)self->array[0].size * RANGE_RESIZE)) {
  ...
  if (self->is_rehashing && self->array[1].num_elements > 0) {
    ...
    if (result != DICT_ERR) {
      ...
      if (old_idx != -1) {
        ...
        self->array[0].elements[old_idx].key =
            TAG_PTR(self->array[0].elements[old_idx].key, DELETED);
