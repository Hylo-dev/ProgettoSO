 ...
 * It relies on an external "tool.h" for the `any` and `let_any` type
 definitions.
...
typedef enum : uint8_t {
  ...
  DELETED =
      2, /**< The slot previously held data but was removed (tombstone). */
...
typedef struct Array {
   ...
   * Aligned to 64 bytes to match common CPU cache lines, reducing false
   sharing.
  ...
  } *elements; // Remove force padding because in big data the cache miss is
               // most frequently
...
typedef struct {
  ...
  size_t rehash_index; /**< Tracks the current progress of the rehashing
                          process. */
 ...
 * Allocates memory for the dictionary and sets up the hash and comparison
 functions.
 ...
 * If the key already exists, the value is updated. If not, a new entry is
 created.
...
bool dict_remove(dict_t *self, let_any key);
